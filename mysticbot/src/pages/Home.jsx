...
export default function Home({ state, showToast }) {
  const { user, canAccess, setCurrentPage, diary, tarotHistory,
          ...
          getEngagementHooks, getContextForClaude, addDailyEnergy, addLuck,
  ...
  return (
    <div>
      ...
      <div style={{ padding: "14px 14px 0", display: "flex", flexDirection: "column", gap: 14 }}>
        ...
        {/* === УТРО/ВЕЧЕР РИТУАЛ === */}
        <MorningEveningRitual user={user} cardRevealed={cardRevealed} dailyCard={dailyCard} setCurrentPage={setCurrentPage} diary={diary} showToast={showToast} addDiaryEntry={state.addDiaryEntry} addLuck={addLuck} />

        {/* === ХУКИ ВОВЛЕЧЁННОСТИ === */}
        <HomeRetentionHooks hooks={getEngagementHooks()} user={user} setCurrentPage={setCurrentPage} investigation={investigation} />
...
// ============================================================
// ХУКИ ВОВЛЕЧЁННОСТИ — блоки на главном экране
// ============================================================
function HomeRetentionHooks({ hooks, user, setCurrentPage, investigation }) {
  const { isWindowOpen, hoursLeft, peakHour, repeatedCard, streakAtRisk, streak, unconfirmed, completedQuizzes } = hooks;

  const blocks = [];
  const TOTAL_QUIZZES = 5;

  // 1. Угроза потери серии (самое срочное — показываем первым)
  if (streakAtRisk && streak >= 2) {
    blocks.push(
      <div key="streak" style={{
        ...
        <div>
          <div style={{ fontSize: 12, fontWeight: 800, color: "#f87171", marginBottom: 2 }}>
            Серия {streak} {pluralizeDays(streak)} под угрозой!
          </div>
          <div style={{ fontSize: 11, color: "var(--text2)" }}>
            Загляни к картам сегодня — не дай пламени угаснуть. ← Погадать
          </div>
        </div>
      </div>
    );
  ...
  // 2. Неподтверждённое предсказание (если есть)
  if (unconfirmed) {
    blocks.push(
      <div key="unconf" style={{
        ...
        <div>
          <div style={{ fontSize: 12, fontWeight: 800, color: "var(--gold2)", marginBottom: 2 }}>
            Предсказание ждёт подтверждения
          </div>
          <div style={{ fontSize: 11, color: "var(--text2)" }}>
            Карты просят обратную связь. Сбылось ли предсказание?
          </div>
        </div>
      </div>
    );
  ...
  // 3. Повторяющаяся карта
  if (repeatedCard) {
    blocks.push(
      <div key="repeat" style={{
        ...
        <div>
          <div style={{ fontSize: 12, fontWeight: 800, color: "var(--accent)", marginBottom: 2 }}>
            «{repeatedCard}» возвращается
          </div>
          <div style={{ fontSize: 11, color: "var(--text2)" }}>
            Эта карта преследует тебя. Новый расклад раскроет её послание.
          </div>
        </div>
      </div>
    );
  ...
  // 4. Открытое энергетическое окно
  if (isWindowOpen) {
    blocks.push(
      <div key="window" style={{
        background: "linear-gradient(135deg,rgba(34,197,94,0.08),rgba(139,92,246,0.06))",
        ...
        <div>
          <div style={{ fontSize: 12, fontWeight: 800, color: "#4ade80", marginBottom: 2 }}>
            Энергетическое окно {user.sun_sign} открыто
          </div>
          <div style={{ fontSize: 11, color: "var(--text2)" }}>
            Сейчас лучшее время для гадания — точность карт максимальна.
          </div>
        </div>
      </div>
    );
    ...
    blocks.push(
      <div key="window-soon" style={{
        ...
        <div style={{ fontSize: 11, color: "var(--text2)" }}>
          Энергетический пик {user.sun_sign} — в <span style={{ color: "var(--gold2)", fontWeight: 700 }}>{peakHour}:00</span> — через {hoursLeft}ч. Лучшее время для гадания.
        </div>
      </div>
    );
  }

  // 5. Расследование — новая часть разблокирована
  if (investigation?.title && (investigation?.progress || 0) < 3) {
    const progress = investigation.progress || 0;
    const remaining = 3 - progress;
    blocks.push(
      <div key="investigation" style={{
  ...
  // 6. Незавершённые опросники
  if (completedQuizzes && completedQuizzes.length > 0 && completedQuizzes.length < TOTAL_QUIZZES) {
    const left = TOTAL_QUIZZES - completedQuizzes.length;
    blocks.push(
      <div key="quizzes" style={{
        background: "rgba(139,92,246,0.07)",
...
// УТРО/ВЕЧЕР РИТУАЛ
// ============================================================
function MorningEveningRitual({ user, cardRevealed, dailyCard, setCurrentPage, diary, showToast, addDiaryEntry, addLuck }) {
  const hour = new Date().getHours();
  const today = new Date().toDateString();
  const [eveningDone, setEveningDone] = useState(() => localStorage.getItem("evening_ritual_done") === today);
  const [eveningText, setEveningText] = useState("");
  const [showEveningInput, setShowEveningInput] = useState(false);

  // Утро (5–11): показываем только если карта ещё не открыта
  if (hour >= 5 && hour < 12 && !cardRevealed) {
    return (
      <div style={{
        background: "linear-gradient(135deg,rgba(245,158,11,0.08),rgba(139,92,246,0.06))",
        border: "1px solid rgba(245,158,11,0.2)", borderRadius: 16, padding: "14px 16px",
      }}>
        <div style={{ fontSize: 13, fontWeight: 800, marginBottom: 6 }}>🌅 Утренний настрой</div>
        <div style={{ fontSize: 12, color: "var(--text2)", lineHeight: 1.6, marginBottom: 10 }}>
          Открой карту дня и задай ей вопрос на сегодняшний день.
          К вечеру — запишем, что сбылось.
        </div>
  ...
  // Вечер (18–23): предлагаем записать итог дня
  if (hour >= 18 && hour < 24 && cardRevealed && !eveningDone) {
    if (showEveningInput) {
      return (
        <div style={{
          ...
          <div style={{ fontSize: 13, fontWeight: 800, marginBottom: 8 }}>🌙 Вечерний итог</div>
          <div style={{ fontSize: 12, color: "var(--text2)", marginBottom: 8 }}>
            Карта дня: {dailyCard?.name}. Как она проявилась?
          </div>
          <textarea
            value={eveningText}
            onChange={e => setEveningText(e.target.value)}
            placeholder="Что произошло сегодня? Как карта дня отозвалась в событиях..."
            style={{
              width: "100%", minHeight: 80, padding: "9px 12px", borderRadius: 10,
              background: "var(--bg3)", border: "1px solid var(--border)", color: "var(--text)",
              fontSize: 12, resize: "none", outline: "none", fontFamily: "inherit", marginBottom: 8,
          ...
          <button onClick={() => {
            ...
            addDiaryEntry?.({
              title: `Вечерний итог · ${dailyCard?.name || "Карта дня"}`,
              text: eveningText,
              mood: "🌙",
            });
            addLuck?.(3, "Вечерний итог");
            localStorage.setItem("evening_ritual_done", today);
            setEveningDone(true);
            setShowEveningInput(false);
            showToast?.("🌙 +3 💫 Итог дня записан!");
          }} style={{
    ...
    return (
      <div style={{
        ...
        border: "1px solid rgba(99,102,241,0.22)", borderRadius: 16, padding: "14px 16px",
        display: "flex", alignItems: "center", gap: 12,
      }}>
        <span style={{ fontSize: 26 }}>🌙</span>
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: 12, fontWeight: 800, marginBottom: 3 }}>Как прошёл день?</div>
          <div style={{ fontSize: 11, color: "var(--text2)" }}>
            Карта «{dailyCard?.name}» была с тобой. Запиши вечерний итог.
          </div>
        </div>
        <button onClick={() => setShowEveningInput(true)} style={{
          fontSize: 11, fontWeight: 700, padding: "7px 12px", borderRadius: 9,
          background: "var(--accent)", color: "white", border: "none", cursor: "pointer", whiteSpace: "nowrap",
        }}>Записать</button>
      </div>
    );
